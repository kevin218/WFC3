...
def zhouramp(rampparams, t, etc):
    """
    This function creates a model that fits the HST 'hook' using the functional form from Zhou et al (2017).

    Parameters
    ----------
    rampparams  : Array of ramp parameters
    t           : Array of time/phase points
    ...
    framenum    = etc[0]
    #istart      = np.where(framenum == framenum.min())[0]
    istart      = np.where(np.ediff1d(framenum) < 0)[0]+1
    t_batch     = t-t[0]
    for ii in istart:
        t_batch[ii:]    = t[ii:]-t[ii]
    #t_batch     = (t-t[0]-t0) % period
    #iorbit2     = np.where(np.ediff1d(t_batch) < 0)[0][0]  #Index of start of second orbit
    iorbit2     = istart[0]
    ...
    return flux - (etas*flux-(Es+deltaEs)*cs)*np.exp(-t_batch*cs) - (etaf*flux-(Ef+deltaEf)*cf)*np.exp(-t_batch*cf)

def recte(rampparams, t, etc):
    """
    This function creates a model that fits the HST 'hook' using the functional form from Zhou et al (2017).
    ...
    obsCounts   = recte.RECTE(cRates, t, exptime, Es, Ef, dEs, dEf)
